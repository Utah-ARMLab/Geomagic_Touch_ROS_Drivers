...
public:
  ...
  void init(OmniState *s) {
    ...
    string_builder.str("");
    string_builder << omni_name << "_sensable";
    sensable_frame_name = string_builder.str();

    for (int i = 0; i < 7; i++) {
      string_builder.str("");
      string_builder << omni_name << "_link" << i;
      link_names[i] = string_builder.str();
    }
...
HDCallbackCode HDCALLBACK omni_state_callback(void *pUserData) {
  ...
  float t[7] = {
    ...
    float(omni_state->rot[0]),
    float(omni_state->rot[1]),
    float(omni_state->rot[2])
    };
  for (int i = 0; i < 7; i++) {
